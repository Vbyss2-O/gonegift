...
const SharedFileUpload = () => {
    ...
    const validation = async () => {
        ...
        try {
            ...
            setMessage("Failed to validate password: " + (error.response?.data?.message || error.message));
    ...
    const decryptPayload = (encryptedBase64, passphrase) => {
        try {
            ...
            if (!decryptedString) {
                throw new Error("Invalid password or corrupted token. Decrypted string is empty.");
            }
    ...
    const showAllFileStoredOfSharedSpace = async () => {
        ...
        try {
            ...
            const decryptedFiles = await Promise.all(
                response.data.map(async (fileMetadata) => {
                    try {
                        ...
                        console.error(`Error processing file ${fileMetadata.uploadFileUrl}:`, fileError);
                        return null;
                    }
                })
            );
            ...
            if (successfulFiles.length === 0) {
                setMessage("No files could be decrypted successfully or found in this shared space.");
            } else if (successfulFiles.length < response.data.length) {
                setMessage(`Successfully retrieved ${successfulFiles.length} out of ${response.data.length} files. Some files failed to decrypt.`);
            } else {
                setMessage(`Successfully retrieved all ${successfulFiles.length} files.`);
            }
        } catch (error) {
    ...
    const encryptFile = async (fileType, fileBlobOrText) => {
        try {
            ...
            for (let i = 0; i < combined.sigBytes; i++) {
                encryptedArray[i] = (combined.words[i >>> 2] >>> (24 - (i % 4) * 8)) & 0xff;
            }
    ...
    return (
        <div className="shared-file-upload-container">
            ...
            <div className="form-section">
                <p className="description">Enter the shared space password to validate and access files.</p>
                <div className="input-group">
                    <label htmlFor="password-input" className="input-label">Shared Space Password:</label>
                    <input
                        id="password-input"
                        type="password"
            ...
            {message && (
                <p
                    className={`message-box ${message.includes("failed") || message.includes("Failed") || message.includes("Error")
                        ? "message-error"
                        : "message-success"
                        }`}
                >
