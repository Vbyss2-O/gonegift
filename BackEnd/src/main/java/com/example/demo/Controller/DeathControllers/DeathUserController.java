...
public class DeathUserController {
    ...
    // here i am adding the two get request methods for checking the size of the
    // files and benificary
    ...
    // below two get controller is for the showing the list of proper benificaries
    // and files
    ...
    @GetMapping("/findHashToken/{token}")
    public ResponseEntity<Void> validateHashuuid(@PathVariable String token) {
    ...
    @GetMapping("/findUUIDByHashuuid/{hashuuid}")
    public ResponseEntity<UUID> findUserIdByHashuuid(@PathVariable String hashuuid) {
        UUID userId = deathUserService.findUserIdXByHashuuid(hashuuid);
        if (userId != null) {
    ...
    public ResponseEntity<String> updateMonitoringStatus(
            @PathVariable UUID id,
            @RequestParam Boolean enabled) {

        Optional<DeathUser> optionalUser = deathUserService.getUserById(id);
        if (optionalUser.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("User not found with ID: " + id);
        }
        DeathUser user = optionalUser.get();


        user.setFlag(enabled);
        deathUserService.saveOrUpdateUser(user);
        return ResponseEntity.ok("Monitoring status updated to: " + enabled);
    }
