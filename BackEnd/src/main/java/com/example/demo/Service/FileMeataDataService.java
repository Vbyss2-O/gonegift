...
public class FileMeataDataService {
    ...
    public List<DeathFiles> getAllFilesBySpecifiUserId(String userID){
       ...
       List<DeathFiles> files = fileMetaDataRepo.findAll();
       List<DeathFiles> wantedusers = new ArrayList<DeathFiles>();
       for (DeathFiles file : files) {
           if(file.getIdOfUser().equals(userID)){
                wantedusers.add(file);
           }
